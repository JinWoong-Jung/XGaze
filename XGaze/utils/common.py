...
@torch.no_grad
def dark_coordinate_decoding(heatmaps, kernel_size = 3, normalize = False):
    """
    Estimates the true coordinates of the max values in the heatmap based on the DARK algorithm.
    
    Args:
        heatmaps (torch.Tensor): The input heatmaps of shape (H, W) or (B, H, W).
        kernel_size (int): The size of the kernel used for gaussian smoothing. Should be the same
        as the std of the gaussian used to create the heatmaps.
        normalize (bool): Specifies whether to normalize the argmax coordinates to [0, 1]. Defaults to False.

        
    Returns:
        A float tensor of shape (B, 2) representing the coordinates of the max values.
    """
    ...
    max_values = heatmaps.amax(dim=(1, 2), keepdim=True)
    max_points = spatial_argmax2d(heatmaps, normalize=False)
    
    # Gaussian Filter
    heatmaps = TF.gaussian_blur(heatmaps.unsqueeze(1), kernel_size).squeeze(1)
    heatmaps = heatmaps * max_values / heatmaps.amax(dim=(1, 2), keepdim=True)

    # Log-likelihood
    heatmaps = heatmaps.clip(min=1e-10).log()

    # Taylor Expansion
    dark_coords = max_points.clone().to(torch.float)
    for i, heatmap in enumerate(heatmaps):
        mx, my = max_points[i]
        if 1 < mx < hm_w - 2 and 1 < my < hm_h - 2:
            # Gradient
            dx = 0.5 * (heatmap[my][mx + 1] - heatmap[my][mx - 1])
            dy = 0.5 * (heatmap[my + 1][mx] - heatmap[my - 1][mx])
            derivative = torch.tensor([[dx],[dy]], device=device)

            # Hessian
            dxdx = 0.25 * (heatmap[my][mx + 2] - 2 * heatmap[my][mx] + heatmap[my][mx - 2])
            dydy = 0.25 * (heatmap[my + 2][mx] - 2 * heatmap[my][mx] + heatmap[my - 2][mx])
            dxdy = 0.25 * (heatmap[my + 1][mx + 1] - heatmap[my - 1][mx + 1] - heatmap[my + 1][mx - 1] + heatmap[my - 1][mx - 1])
            hessian = torch.tensor([[dxdx, dxdy],[dxdy, dydy]], device=device)

            if dxdx * dydy - dxdy ** 2 != 0:
                offset = hessian.inverse() @ derivative
...
def find_bounding_box(mask):
    ...
    Args:
        mask: a binary tensor of shape (N, 1, H, W) representing the mask, where N is the number of masks,
              H is the height of the mask, and W is the width of the mask.
    
    Returns:
        A tensor of shape (N, 4) representing the minimal bounding boxes containing the masks,
        in the format [xmin, ymin, xmax, ymax].
    """
    non_zero_indices = torch.nonzero(mask.squeeze(), as_tuple=True)  # Find non-zero indices in the mask
    
    if non_zero_indices[0].numel() == 0:
        # Handle case where there are no non-zero pixels in the mask
        return torch.zeros((mask.shape[0], 4), dtype=torch.float32, device=mask.device)
    
    ymin, xmin = non_zero_indices[0].min(), non_zero_indices[1].min()
    ymax, xmax = non_zero_indices[0].max(), non_zero_indices[1].max()
    
    bounding_boxes = torch.stack([xmin, ymin, xmax, ymax]).float()
